  ...
  before_filter :authenticate_user!, :except => [:create, :signin, :signup, :newaccount, :failure]
    ...
    def newaccount
      if params[:commit] == "Cancel"
        ...
        @newuser.password = session[:authhash][:password]  
        @newuser.services.build(:provider => session[:authhash][:provider], :uid => session[:authhash][:uid], :uname => session[:authhash][:name], :uemail => session[:authhash][:email], :upassword => session[:authhash][:password])
    ...
    def create
      ...
      if omniauth and params[:service]
        ...
        if service_route == '1facebook'
          ...
          omniauth['extra']['user_hash']['id'] ? @authhash[:uid] =  omniauth['extra']['user_hash']['id'].to_s : @authhash[:uid] = ''
          omniauth['provider'] ? @authhash[:provider] =  omniauth['provider'] : @authhash[:provider] = ''  
        elsif ['google', 'yahoo', 'twitter', 'myopenid', 'open_id', 'tsina'].index(service_route) != nil
          omniauth['user_info']['email'] ? @authhash[:email] =  omniauth['user_info']['email'] : @authhash[:email] = 'services@services.com'
          omniauth['user_info']['name'] ? @authhash[:name] =  omniauth['user_info']['name'] : @authhash[:name] = 'services'
          omniauth['user_info']['password'] ? @authhash[:password] =  omniauth['user_info']['password'] : @authhash[:password] = 'services'  
        ...
        if @authhash[:uid] != '' and @authhash[:provider] != ''
         puts "uid: #{@authhash[:uid]},  provider: #{@authhash[:provider]}"
         puts "service: #{Service.all}"
          ...
          if user_signed_in?
            if auth
              ...
              current_user.services.create!(:provider => @authhash[:provider], :uid => @authhash[:uid], :uname => @authhash[:name], :uemail => @authhash[:email], :upassword => @authhash[:password])
